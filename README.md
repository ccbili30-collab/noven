...
GWP（大型世界工程，命令为 `/growth_world_pro`）不是普通的一键内容生成器，而是一条可以持续很久的深度世界构建流程。它会先建立路线、资料边界和十二层世界蓝图，再逐步物化人物、文明、地点、事件和故事等内容。根据种子质量、项目规模和运行时长，成果可以继续扩展到数十万字量级，但不承诺固定字数或完成时间。

### 不建议从完全空白直接启动

GWP 可以从空项目开始，但已有积累通常会带来更明确、更稳定的结果。启动前建议至少准备一项：
...
> 根据项目里已有的灰冠设定和参考图片，建立一个机械宗教与极端环境共存的硬科幻世界。保留灰烬、圣骸和阶层冲突，不要使用魔法解释技术。

相比只输入“生成一个科幻世界”，带有种子、边界和创作愿望的要求更容易形成有辨识度的世界。

### 根据任务规模选择能力

| 你的目标 | 推荐方式 |
| --- | --- |
| 写一篇短文、设计角色、研究资料或生成图片 | 普通会话或对应 Skill |
| 建立一个可继续扩展的完整世界 | `/growth_world` |
| 构建长期、复杂、大规模世界工程 | `/growth_world_pro` |

如果只需要地图、人物、因果链或漫画，不建议为了这一项作品启动 GWP。

### 为什么地图、漫画等作品不全部放在 GWP 里

为了减轻超长流程的上下文和运行负担，地图、人物群像、因果链、漫画等附属作品已经从 GWP 主流程中拆出。GWP 优先建立稳定的世界基础，之后可以根据需要分别调用专门 Skill：
...
拆分不会减少这些能力，而是避免一个超长任务同时承担所有工作，让每类作品使用更专门、负担更小的创作流程。

### “准备中”不一定表示卡死

GWP 的准备阶段可能持续较长时间。它可能正在读取大型项目、检查既有文件、判断应该新建还是续建世界、恢复世界蓝图，或者整理上下文。准备时间较长本身不等于失败。

如果界面仍有新的工具活动、阶段信息或文件变化，可以继续等待；如果同一条错误持续重复、自我修复次数不断增加，并且项目长时间没有任何新变化，则说明恢复没有收敛，不应无限等待。

遇到这种情况：

1. 先暂停或结束原有 GWP，避免两个主任务同时修改同一项目；
2. 新建项目会话；
3. 明确要求诺文“读取现有项目并继续 GWP”，不要重新创建同名世界；
4. 如果只是软件会话异常，也可以使用左下角的 **恢复诺文** 重启软件后再继续。

### 真实案例：等待很久后，世界仍然重新生长起来

在一次“视频种子世界”实测中，GWP 进入全世界蓝图阶段后，界面连续显示了约 11 分钟的“正在自动修复”。当时它遇到的不是单一报错，而是多项蓝图校验问题：部分层名不精确、对象类型或父级关系不符合要求，还有多个世界层尚未达到最低正文条目数量。

![GWP 正在自动修复蓝图问题](docs/assets/readme/noven-gwp-self-recovery.png)

这次运行并没有停在错误界面：GWP 依据已经保存的项目状态继续检查和补全蓝图，随后成功提交蓝图、完成确认与冻结，并进入正式的自由物化阶段。它等待得很久，但最终仍然跑了起来，而且保留了此前已经形成的真实文件。

这个案例说明两件事：

- 长时间“准备中”或“自动修复”不一定意味着任务已经死亡；只要工具活动、阶段信息或项目文件仍有变化，就可以继续观察。
- 自我修复不等于无限等待。如果同一错误持续增加、项目长时间没有变化，或者出现余额、密钥、权限等硬性阻塞，仍应暂停旧任务，解决原因后从新会话读取现有项目继续。

> 判断 GWP 是否仍在工作，不只看等待时间，更要看它是否还在产生新的工具活动、阶段进度或真实文件变化。

### GWP 的恢复能力与边界

GWP 会以真实项目文件和已经保存的世界状态为依据，检查已有产物、补交缺失的阶段进度、避免重复覆盖，并对部分蓝图阶段冲突重新检查和修正。

但自我修复不是无限重试，也不能解决所有外部问题。以下情况仍需用户处理：
...
GWP 运行时建议保持诺文开启。大量移动、删除或重命名项目文件前，先暂停任务。你可以随时查看已经生成的文件，不必等到整个世界全部完成。

> **先用内容与灵感种下种子，再让 GWP 生长世界；先让世界稳定，再用独立 Skill 创作地图、人物、因果链、小说和漫画。**
