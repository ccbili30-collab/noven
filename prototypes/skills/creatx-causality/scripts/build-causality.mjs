...
const SKILL_ID = "creatx-causality"
const OUTPUT_MARKER = ".noven-causality.json"
const LAYER_ORDER = [
  "核心规则与边界",
  "宇宙、自然与地理",
  "生态、资源与物种",
...
main().catch((error) => {
  ...
  console.error(`CAUSALITY_ERROR [${code}] ${message}`)
  process.exitCode = 1
})
...
async function main() {
  ...
  const projectRoot = await resolveRequiredDirectory(options.projectRoot, "--project-root")
...
function printHelp() {
  console.log(`Usage:
  node build-causality.mjs --project-root <path> [options]

Options:
  --world-root <worlds\\name>  Select one world when the project contains several
  --goal-id <goal_id>          Select one Growth Goal
  --output <path>              Override the default <world-root>\\世界因果图
  --help                       Show this help`)
}

async function resolveRequiredDirectory(value, label) {
  if (!value?.trim()) throw failure("ARGUMENT", `${label} 不能为空`)
  const pathValue = resolve(value)
  const info = await lstat(pathValue).catch(() => undefined)
  if (!info?.isDirectory() || info.isSymbolicLink()) throw failure("ARGUMENT", `${label} 必须是真实目录`)
...
async function discoverCandidates(projectRoot, goalId) {
  ...
  const canonicalGoalsRoot = await realpath(goalsRoot).catch((error) => {
    if (error?.code === "ENOENT") throw failure("NO_GROWTH_STATE", `找不到 Growth Goal 目录：${goalsRoot}`)
    throw error
  })
  if (!isWithin(canonicalGoalsRoot, projectRoot)) throw failure("PATH_ESCAPE", "Growth Goal 目录越出项目根")
  const goalDirectories = await readdir(canonicalGoalsRoot, { withFileTypes: true })
  ...
  for (const directory of selectedDirectories) {
    const goalRoot = join(canonicalGoalsRoot, directory.name)
...
function resolveOutputRoot(value, projectRoot, worldRoot) {
  const outputRoot = value ? resolve(projectRoot, value) : resolve(projectRoot, ...worldRoot.split("/"), "世界因果图")
  if (outputRoot === projectRoot) throw failure("OUTPUT_UNSAFE", "输出目录不能是项目根")
  if (!isWithin(outputRoot, projectRoot)) throw failure("OUTPUT_UNSAFE", "输出目录必须位于项目根内")
...
async function assertSafeOutput(outputRoot, projectRoot) {
  ...
  if (!isWithin(canonicalParent, projectRoot)) throw failure("OUTPUT_UNSAFE", "输出目录的真实父路径必须位于项目根内")
  if (!await exists(outputRoot)) return
  const info = await lstat(outputRoot)
  if (!info.isDirectory() || info.isSymbolicLink()) throw failure("OUTPUT_UNSAFE", "输出路径必须是真实目录，不能是链接")
...
async function buildAtlas(selected, projectRoot) {
  ...
  const worldNodes = source.nodes.filter((node) => toPosix(node?.path ?? "").startsWith(worldPrefix))
  const worldNodeById = new Map(worldNodes.map((node) => [node.id, node]))
  const causalRelations = source.relations.filter((relation) => relation?.type === "causes" && worldNodeById.has(relation.from) && worldNodeById.has(relation.to))
  if (!causalRelations.length) throw failure("NO_CAUSALITY", `${selected.worldRoot} 没有明确登记的 causes 因果关系`)
  const causalNodeIds = new Set(causalRelations.flatMap((relation) => [relation.from, relation.to]))
  const causalPaths = new Set(worldNodes.filter((node) => causalNodeIds.has(node.id)).map((node) => toPosix(node.path)))
  const nodes = worldNodes.filter((node) => causalPaths.has(toPosix(node.path)))
  ...
  const works = await Promise.all([...workByPath.values()].map(async (work) => {
    ...
    try {
      ...
      if (!isWithin(canonicalPath, projectRoot)) throw failure("PATH_ESCAPE", `项目文件真实路径越界：${work.path}`)
      return { ...work, content: await readFile(canonicalPath, "utf8") }
    } catch (error) {
  ...
  const facts = selected.kind === "materialization" ? nodes
    .filter((node) => node.layer === "正文事实" && causalNodeIds.has(node.id) && finalWorkByPath.has(toPosix(node.path)))
    .map((node) => {
  ...
  causalRelations.forEach((relationValue) => {
...
function createSummary(selected, atlas, outputRoot, invalid) {
  return {
    ...
    warnings: [
      "实验版使用本地 Canvas 渲染且不依赖外部网络；它表达已登记因果理解，不是客观证明或模拟器。",
